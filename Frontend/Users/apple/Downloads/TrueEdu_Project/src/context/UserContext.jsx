...
export const UserProvider = ({ children }) => {
  const getInitialUser = () => {
    const storedUser = localStorage.getItem('trueed_user');
    if (storedUser) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('trueed_user', JSON.stringify(user));
  }, [user]);
  ...
  const updateUser = (updates) => {
    setUser((prevUser) => {
      ...
      localStorage.setItem('trueed_user', JSON.stringify(newUser));
      return newUser;
    });
  };

  const resetUser = () => {
    localStorage.removeItem('trueed_user');
    setUser({ ...defaultUser });
  };
