...
const TutorCard = ({ tutor }) => {
  ...
  return (
    <div className={`bg-white rounded-brand border border-gray-200 overflow-hidden hover:-translate-y-0.5 hover:shadow-md transition-all duration-200 flex flex-col h-full group relative ${tutor.badge === 'Expert' ? 'border-t-4 border-t-amber-400' : ''}`}>
      {/* Save Heart */}
      <button 
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="flex items-center gap-2 mb-4">
          {badge && (
            <span className={`text-xs px-2.5 py-1 rounded-md font-bold shadow-sm flex items-center gap-1.5 ${badge.bg}`}>
              <span>{badge.icon}</span> {tutor.badge}
            </span>
          )}
          <div className="flex items-center gap-1 text-amber">
            {[...Array(5)].map((_, i) => (
              <i key={i} className={`fa-star text-xs ${i < Math.floor(tutor.rating) ? 'fa-solid' : i < tutor.rating ? 'fa-solid fa-star-half-stroke' : 'fa-regular'}`} />
            ))}
            <span className="text-xs font-bold text-navy ml-1">{tutor.rating}</span>
            <span className="text-xs text-muted font-medium ml-1">({tutor.reviews})</span>
          </div>
        </div>
      ...
      <div className="p-5 pt-4 bg-slate-50/50 border-t border-slate-100 mt-auto">
        ...
        <div className="flex flex-col gap-2">
          ...
          <Link
            to={`/teacher/${tutor.name.toLowerCase().replace(/\s+/g, '-')}`}
